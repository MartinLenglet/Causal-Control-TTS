function figure_counter = plot_categorical_controllability_by_layer( ...
...
fp = get(gcf,'Position');
fp(3:4) = [1250 430];
set(gcf,'PaperSize',[44 15])
set(gcf,'Position',fp)
set(gcf,'Renderer', 'painters');

posPlot = plotPosition(1,4,[0.035 0.0 0.1 0.18],[0.02 0.01 0.00 0.07]);
p = panel();
...
for i_model = 1:nbr_models
    ...
    for i_param = list_params_supra_segmental
        ...
        hold on;
        plot(1:nbr_layers, max_pred_by_layer(i_param, :, i_model), 'LineStyle', '--', 'LineWidth', linewidth_control, 'Color', legend_by_param{index_param, 2});
        plot(1:nbr_layers, min_pred_by_layer(i_param, :, i_model), 'LineStyle', ':', 'LineWidth', linewidth_control, 'Color', legend_by_param{index_param, 2});
        plot(xlimits, ylimits_targets(1)*ones(1, 2), '-.', 'LineWidth', linewidth_lim,'HandleVisibility','off','Color',pcol.gipsaPSD);
        plot(xlimits, ylimits_targets(2)*ones(1, 2), '-.', 'LineWidth', linewidth_lim,'HandleVisibility','off','Color',pcol.gipsaPSD);
    ...
    if i_model == nbr_models
        ...
        lgnd = legend({'unbiased'; 'max achievable'; 'min achievable    |'; 'R^{2}_{L}: 0.1'; 'R^{2}_{L}: 0.5'; 'R^{2}_{L}: 1'}, 'Location','southeast','interpreter','tex','Orientation','horizontal');
        lgnd.Position(1) = 0.3;
